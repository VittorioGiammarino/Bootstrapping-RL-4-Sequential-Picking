...
class ActionHead(nn.Module):
  ...
  def __init__(self, inp_dim, size, layers, units, act=nn.ELU, dist='onehot', init_std=0.0, min_std=0.1, action_disc=5, temp=0.1, outscale=0):
...
class DreamerV2Agent(nn.Module):
  def __init__(self, input_shape, action_shape, action_pixel_shape, device, dataset, traindir, config):
    super(DreamerV2Agent, self).__init__()

    self.action_shape = action_shape[0]
    self.num_actions = action_pixel_shape
    self.device = device
    self._config = config
    self._step = count_steps(traindir)

    self.input_shape = input_shape
    self.input_image_shape = input_shape[:2]
    self.num_expl_steps = self._config.num_expl_steps
    ...
    self.world_model = WorldModel(self._step, self.num_actions, config)
    self.task_behavior = ImagBehavior(config, self.num_actions, self.world_model, actor_entropy, 
  ...
  def act(self, input_image, agent_state, xyz, step, eval_mode):

    obs = {}

    if agent_state is None:
      latent = self.world_model.dynamics.initial(1)
      action = torch.zeros((1, self.num_actions)).to(self.device)
    else:
      latent, action = agent_state

    obs['image'] = (torch.Tensor(input_image.copy()) / 255.0 - 0.5).unsqueeze(0).to(self.device)
    ...
    elif step < self._config.num_expl_steps:
    ...
    if self._config.actor_dist == 'onehot_gumble':
      action = torch.one_hot(torch.argmax(action, dim=-1), self.num_actions)

    action = self.exploration(action, eval_mode)
    state = (latent, action)
    policy_output = {'action': action.cpu().numpy()[0], 'logprob': logprob.cpu().numpy()[0]}

    action_index = np.argmax(policy_output['action'])
    action_pixel_space = np.unravel_index(action_index, shape=self.input_image_shape)

    picking_pixel = action_pixel_space[:2]
    picking_y = int(picking_pixel[0])
    picking_x = int(picking_pixel[1])

    action_numpy = policy_output['action']

    return action_numpy, picking_y, picking_x, state

  def exploration(self, action, eval_mode):
    amount = self._config.expl_amount if not eval_mode else self._config.eval_noise
  ...
  def pretrain(self, steps):
    ...
    for _ in range(steps):
      ...
      mets_behavior = self.task_behavior._train(start, reward)[-1]
  ...
  def update(self, step):
    ...
    reward = lambda f, s, a: self.world_model.heads['reward'](self.world_model.dynamics.get_feat(s)).mode()
    metrics.update(self.task_behavior._train(start, reward)[-1])
